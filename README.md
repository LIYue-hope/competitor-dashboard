...
- [x] 调整 LLM 输出每日总结的提示词、尽可能避免规则文本的输出：综述要求 100–140 字、
  补充具体素材，配合缓存失效、限流重试与主备模型回退（已实现，2026-08-26）
...
- **热门动态** — 按发行商分组（腾讯 / 网易 / 米哈游 / 鹰角、库洛、叠纸 / 其他），
  看各游戏官方近 7 天的版本前瞻、更新公告、新活动、赛事等动态；米哈游分组新增
  星布谷地，其他分组新增蓝色星原：旅谣和闪耀吧！噜咪。
- **游戏资讯** — 3DMGame / 游侠网 / 游民星空 / GameLook / 游资网 五个来源，
  ...
  按分页条数显示时，可通过列表底部页码浏览其余新闻，以兼顾长列表浏览与页面性能。资讯列表下方展示各网站每日新增资讯曲线，横轴为日期、纵轴为当天新增条数，数据从 2026-09-01 起每日持久化更新；曲线支持近一周、近半月、近一月、近半年与总数据筛选，默认近一周，资讯刷新成功后会同步采用刚拉取的数值。
- **历史数据** — 侧栏一级板块；从 2026-08-31 起累计展示历史热度榜和历史游戏资讯榜：
  热度榜在每周一上周总览成稿后更新，资讯榜每天随资讯采集增量更新；前端可按
  20 / 50 / 100 条分页查看，两个榜单的显示条数可分别筛选

左侧一级板块点击后只切换右侧内容并自动展开当前组，不改变页面滚动位置；点击标题右侧的 `>` 可展开或收起该板块的小标题。点击小标题或滚动到对应内容时，该小标题以浅蓝色高亮（例如「历史数据」可展开「历史热度榜」和「历史游戏资讯榜」）。

页面顶部的「数据概览」固定在左右内容区域的共同上方，不会随左侧板块切换而变化，但会随页面滚动离开视口。概览以已完成采集的昨日数据为基准，展示昨日资讯总量、较前日变化、近 7 日峰值来源、未来 7 日新游、官方动态数和最新数据时间；其下另列未来 7 日内被来源标记为「挂机/搬砖」的游戏。该列表按游戏和上线日期合并，同日多平台以顿号列出；不同上线日期则分别列出对应的平台和日期，点击游戏名可前往详情页。历史热度榜按各游戏历史最高的一周热度排序：只有本周热度更高时才更新该游戏记录；历史游戏资讯榜按累计资讯数量排序。两个历史榜均使用相同的紧凑页码分页。

资讯趋势图默认展示各来源每日新增曲线与不可关闭的「总量」曲线；可按需勾选「各源占比」和「7 日移动均线」。当任一来源当日缺失时，总量和均线会标记为数据不完整并断线，占比不会将该来源误作 0。勾选各源占比后，曲线图下方会出现当前范围最新 14 个数据日的 100% 堆叠条，并保留每个来源的条数和占比提示。地址栏会同步当前板块及资讯来源、分类、关键词、日期筛选和趋势选项，复制链接可分享同一视图；经营概览中的「导出当前页面 Excel」会下载 `页面名-YYYYMMDD-HHMMSS.xls`（北京时间）格式的多工作表工作簿：各来源或小标签分别写入独立工作表，历史数据始终包含「历史热度榜」和「历史游戏资讯榜」两个工作表。

顶栏有深浅色切换（跟随系统，可手动覆盖，记在 `localStorage`），以及当前已加载数据中可用的最新采集时间戳。
吸顶栏和 Tab 栏用半透明磨砂底；不支持 `backdrop-filter` 的浏览器回退为不透明底色。
新游监测、热门动态和游戏资讯的吸顶 Tab 栏会在滚动折叠或展开时平滑过渡，并遵从系统的“减少动态效果”设置。
鼠标悬停或通过键盘聚焦 Tab 标签、新闻条目、官方动态和游戏卡片等具体内容时，该条目会平滑放大至 1.05 倍，以突出当前焦点；游戏资讯和热门动态的条目带完整左右框线；顶栏和吸顶 Tab 栏本身不会整体缩放。
...
`weekly_history.json` 自 2026-08-31 起分开整理两个榜单：历史热度榜只在每周一上周总览
成功成稿后更新一次：每款游戏只保留热度最高的一周，本周未超过既有热度时保留原记录；历史游戏资讯榜每天读取各资讯源的滚动保留窗口并增量合并，
不受热度榜门槛或周报 Top 10 限制。资讯榜会将同名游戏合并、累计资讯数量，并显示该游戏首篇资讯日
至末篇资讯日。`news_history` 保留完整累计底稿，页面的 `news_ranking` 仅派生前 100 款，避免榜外游戏
再次上榜时丢失既有资讯数；`news_days` 记录每日整理日期。已有 v2 底稿会保留并迁移到每日整理逻辑；
后续按文章 URL（缺失时回退来源、日期与标题）去重，避免重跑或滚动窗口重叠导致重复累计。

如需仅修复已有 `weekly_history.json` 的资讯展示排序（不读取采集源，也不改动 `news_history` 累计底稿），可运行
`python scripts/summarize_week.py --repair-news-history-ranking`；该命令只在 `news_ranking` 与当前排序规则不一致时写回文件。
...
Vue 3.4 + Vite 5，无状态管理库、无 UI 框架，组件直接 `fetch` 对应 JSON；浏览器本地生成兼容 Excel 的 SpreadsheetML 多工作表 `.xls` 报告，无额外导出依赖。
...
`.github/workflows/crawl.yml`，每天 UTC 22:00（北京时间次日 06:00）定时触发，也可手动 `workflow_dispatch`。
