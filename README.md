...
页面顶部的「数据概览」固定在左右内容区域的共同上方，不会随左侧板块切换而变化，但会随页面滚动离开视口。概览以已完成采集的昨日数据为基准，展示昨日资讯总量、较前日变化、近 7 日峰值来源、未来 7 日新游、官方动态数和最新数据时间。历史热度榜显示最新周相对前一周的排名/热度变化及连续上榜周数；首周或无前周对照时会标注为新上榜。
...
成功成稿后追加一次，随后随周报冻结；历史游戏资讯榜每天读取各资讯源的滚动保留窗口并增量合并，
