...
页面顶部的「数据概览」固定在左右内容区域的共同上方，不会随左侧板块切换而变化，但会随页面滚动离开视口。概览以已完成采集的昨日数据为基准，展示昨日资讯总量、较前日变化、近 7 日峰值来源、未来 7 日新游、官方动态数和最新数据时间。历史热度榜按各游戏历史最高的一周热度排序：只有本周热度更高时才更新该游戏记录；历史游戏资讯榜按累计资讯数量排序。两个历史榜均使用相同的紧凑页码分页。
