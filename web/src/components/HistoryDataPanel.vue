...
const heatRows = computed(() => {
  // 兼容旧版逐周快照：数据文件完成下一次写入前，页面也应立即只显示每款游戏的最高热度。
  const bestByName = new Map()
  for (const row of props.data?.heat_ranking || []) {
    if (Number(row.heat_score) <= 0) continue
  ...
  return [...bestByName.values()].sort((a, b) => Number(b.heat_score) - Number(a.heat_score) || Number(b.media_count) - Number(a.media_count) || a.name.localeCompare(b.name))
})
...
// 两个历史榜统一使用与游戏资讯榜相同的紧凑页码。
function pageOptions(rows, page, pageSize) {
  const total = pages(rows, pageSize)
...
const heatPageOptions = computed(() => pageOptions(heatRows.value, heatPage, heatPageSize))
const newsPageOptions = computed(() => pageOptions(newsRows.value, newsPage, newsPageSize))
  ...
  <div class="history-data">
    ...
    <template v-else>
      <div class="history-note-row">
        <p class="hint">历史数据自 08-31 起累计：热度榜保留各游戏历史最高的一周热度；游戏资讯榜每日更新。</p>
      </div>
      ...
      <section id="history-heat-ranking" class="history-section history-heat-section">
        <div class="card-head sticky-heading">
          ...
          <span class="stamp">按历史最高热度排序</span>
        ...
        <nav v-if="heatRows.length > heatPageSize" class="news-pager" aria-label="历史热度榜分页">
          <button class="icon-btn" :disabled="heatPage === 1" @click="heatPage--">上一页</button>
          <template v-for="(page, index) in heatPageOptions" :key="page || `gap-${index}`">
            <span v-if="page === null" class="pager-gap" aria-hidden="true">…</span>
            <button
              v-else
              class="icon-btn pager-page"
              :class="{ active: page === heatPage }"
              :aria-current="page === heatPage ? 'page' : null"
              :aria-label="`第 ${page} 页`"
              @click="heatPage = page"
            >{{ page }}</button>
          </template>
